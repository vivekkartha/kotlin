        ...
        when {
            dep.isFile -> {
                ...
                if (dep.extension == "jar") {
                    try {
                        with(JarFile(dep)) {
                            if (getJarEntry(SCRIPT_DEFINITION_MARKERS_PATH) != null) {
                                for (template in entries()) {
                                    if (!template.isDirectory && template.name.startsWith(SCRIPT_DEFINITION_MARKERS_PATH)) {
                                        ...
                                        val templateClass = getJarEntry("${templateClassName.replace('.', '/')}.class")
                                        if (templateClass == null) {
                                            ...
                                            yield(
                                                LazyScriptDefinitionFromDiscoveredClass(
                                                    getInputStream(templateClass).readBytes(),
                                                    templateClassName, listOf(dep) + extractClasspath(defaultScriptDefinitionClasspath),
